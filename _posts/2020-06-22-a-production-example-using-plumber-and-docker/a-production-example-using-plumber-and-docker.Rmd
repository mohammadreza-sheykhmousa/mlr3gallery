...
Let's create a toy pipeline that we want to use to predict the `Species` of the well known `r ref("mlr_tasks_iris", "iris")` `r ref("Task", "Task")`:

```{r}
task = tsk("iris")
```

Regarding modeling, we will keep it very simple and use the `r ref("mlr_learners_classif.rpart", "rpart learner")`. Missing numerical features will be imputed by their median via `r ref("PipeOpImputeMedian", "PipeOpImputeMedian")`.
...
```{r}
newdata = data.table(Petal.Length = NA, Petal.Width = 3.14,
  Sepal.Length = 6.96, Sepal.Width = 0.12)
...
resp = httr::POST(url = "http://127.0.0.1:1030/predict_species",
...
Finally, you can proceed to deploy your container to an Amazon Web Service or an Azure Cloud. For the latter, the package `r cran_pkg("AzureContainers")` is especially helpful. If you do plan to do this note that the `plumber` service aboe is exposed over HTTP, and there is no authentication layer making it insecure. You may think about adding a layer of authentification and restricting the service to HTTPS.
